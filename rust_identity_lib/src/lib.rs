...
pub static API_ENDPOINT: &str = "http://127.0.0.1:14265";
pub static FAUCET_ENDPOINT: &str = "http://localhost:8091";
...
pub async fn create_did(
    ...
    let address: Address = get_address_with_funds(
        ...
        secret_manager,
        FAUCET_ENDPOINT
    ).await.context("failed to get address with funds")?;

    let network_name: NetworkName = client.network_name().await?;

    let (document, fragment): (IotaDocument, String) = create_did_document(
        &network_name,
        storage
    ).await?;
...
pub async fn get_address_with_funds(
    ...
    request_faucet_funds(client, address, faucet_endpoint).await.context(
...
pub async fn get_address(
    ...
    let mnemonic = bip39::wordlist
        ::encode(random.as_ref(), &bip39::wordlist::ENGLISH)
    ...
    let address: Bech32Address = secret_manager.generate_ed25519_addresses(
...
async fn request_faucet_funds(
    ...
    tokio::time
        ::timeout(std::time::Duration::from_secs(45), async {
            loop {
                tokio::time::sleep(std::time::Duration::from_secs(5)).await;

                let balance = get_address_balance(client, &address).await.context(
...
async fn get_address_balance(client: &Client, address: &Bech32Address) -> anyhow::Result<u64> {
    let output_ids = client.basic_output_ids(
        vec![
